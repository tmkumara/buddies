...
See `.env` for defaults (copy from `.env.example` if needed). You can update any values before running Docker Compose.

Key values:
- `MYSQL_DATABASE`, `MYSQL_USER`, `MYSQL_PASSWORD`
- `JWT_SECRET`, `JWT_ACCESS_MINUTES`, `JWT_REFRESH_DAYS`
- `BOOTSTRAP_ADMIN_EMAIL`, `BOOTSTRAP_ADMIN_PASSWORD`
- `VITE_API_BASE_URL` (frontend -> backend base URL, default `http://localhost:8080`)
